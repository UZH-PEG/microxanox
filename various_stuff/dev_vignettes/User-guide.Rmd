...
# (Default) conditions that must be defined
...
At present there are two functions available, one for single strain simulations, and one for multi-strain simulations. Both can add noise to a state variable, and/or assure minimum abundance is not exceeded.
...
Once all the conditions are assigned to the default objects, then the `run_simulation` function will pick them up without being explicitly given them:
