...
The ordinary differential equations for the rates of change are specified in the function `bushplus_dynamic_model_strains()`. This is an augemented version that can handle multiple strains within each of the three functional groups. The basic version that has only one strain per functional group is `bushplus_dynamic_model()`.
...
The event definition is made in the form of a function. The default definition is in the function `default_event_definition()`
...
# Repeat the results above using the functions that allow simulation of multiple strains per functional group

## Test with one strain per functional group
...
### Test with N strains per functional group
