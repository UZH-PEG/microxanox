...
```
All parameter values are set initially to NA and need to be set. This can either be done by providing the parameters to be set as named arguments to the function `new_runsim_parameter()`:
...
```{r}
parameter_set1$strain_parameter <- new_strain_parameter(n_CB = 1,
                                        values_CB = "bush",
                                        n_PB = 1,
                                        values_PB = "bush",
                                        n_SB = 1,
...
Here, $n_CB$, $n_PB$, and $n_SB$ are the number of strains within each functional group and $values_CB$ (etc) are the parameter values of the strains. The final argument sets the initial state of the system to a preset from the Bush et al (2007) publication.
...
```{r}
parameter_set1$initial_state
```

is used to start a simulation, while

```{r}
parameter_set1$strain_parameter$initial_state
```

is by the function `ss_by_a_N()` (this is a function used to get stable states).

A good way to set these is:
...
Specified by giving a vector of values of $a_O$. These are then automatically spaced equally along the duration of the simulation. Here we specify only two values of $a_O$, and so these are the start and end vlaues, and they are the same. Hence there is no temporal change in $a_O$.
