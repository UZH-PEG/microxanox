...
anoxic_start_sim_results <- run_simulation(parameter_values = bush_anoxicstart_1strain,
                                      initial_state = bush_anoxic_1_strain$initial_state)
```


```{r eval = run_steadystate_sims, echo = FALSE}
save(anoxic_start_sim_results, file = "data/anoxic_start_sim_results.Rda")
```

The convenience function `plot_dynamics_strains` can be used to easily visualise the dynamics of a simulation:

```{r echo = FALSE}
load(file = "data/anoxic_start_sim_results.Rda")
```
...
```{r eval = run_steadystate_sims, echo = FALSE}
save(oxic_start_sim_results, file = "data/oxic_start_sim_results.Rda")
```


```{r echo = FALSE}
load(file = "data/oxic_start_sim_results.Rda")
```
