...
To now apply the changes done in log10a_series, onto `10aO_series` and the inverse `log10aS_series` in `parameter`we need the helper function `set_diffusivities`. This function sets the `log10aO_series` and `log10aS_series` (inversed) in parameter. It is absolutely necessary to use this function WHENEVER the diffusivity pattern of change is modyfied! The diffusivity pattern of change is always modifyed by changing `parameter$log10a_series`.
...
Next, we will run a symmetric simulation driven by the vector we just defined. For more realistic simulations we can increase the `parameter$sim_duration`, but doing so can lead to simulation taking enourmous times, if the used computer has low parallelized computation power. Thus, we will leave it short (and not in stable state) for presentation porpuses.

The simulation is simply run by calling the function `run_simulation_symmetric` and assigning its result to a variable.

```{r  eval = !file.exists(mdat("simulation_time_dynmaics_symmetric.RDS"))}
...
```{r eval = !file.exists(mdat("simulation_symmetric_temporal.RDS"))}
...
```{r eval = !file.exists(mdat("simulation_asymmetric_hOSB.RDS"))}
result_trait.asym <- run_temporal_ssfind_symmetric(parameter) 
```

### Diffusivity asymmetry

We can also modify the pattern of diffusivity pattern to be asymmetric. To change the diffusivity pattern we use the `asym_factor` of the helper function `set_diffusivities`, you can explore the relationship between the `asym_factor` and the asymmetric diffusivity pattern. It is also important to remember to make trait values symmetric again.
...
parameter$strain_parameter$SB$h_O_SB <- parameter$strain_parameter$CB$h_O_CB
# change the pattern 
...
```{r eval = !file.exists(mdat("simulation_asymmetric_stressor.asym0.6.RDS"))}
result_stressor.asym <- run_temporal_ssfind_symmetric(parameter)
```

## Quantificataion of asymmetry

The function `get_symmetry_measurements` returns different values used to quantify the (a)symmetry.

```{r}
asym_measures <- get_symmetry_measurements(result_stressor.asym)
```

## Visualization of asymmetry

The (a)symmetry measures obtained from `get_symmetry_measurements` can directly visualized by using `plot_asymmetry_measures`. This convenience function serves for graphical orientation of the measurements. As a first argument it takes the result data frame and as the second one we need to define the species to plot, which is either a bacteria or a chemical.

```{r}
plot_symmetry_measurements(result_stressor.asym)
```
...
```{r}
plot_trajectory_symmetry(result_stressor.asym)
```


Thank you for conducting this user guide, if there are still remaining questions or you find bugs of any kind I would be pleased if you [reached out](mailto:compbio.baertschi@gmail.com)!
