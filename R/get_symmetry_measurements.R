...
get_symmetry_measurements <- function(res){
  ...
  ## prepare storage df
  measures <- data.frame(anox_TP = rep(NA, length(all_species)), 
  ...
  for (i in 1:length(all_species)){
    ...
    for (dir in c("oxic", "anoxic")){
      ...
      temp <- data.frame(df = df[,all_species[i]], 
                         aO1 = dx1[, "aO"],
                         aO2 = dx2[, "aO"],
    ...
    # anoxic recovery
    area_anox <- DescTools::AUC(x = res %>% filter(recovery == "anoxic") %>% pull("aO"),
    ...
    # oxic recovery
    area_ox <- DescTools::AUC(x = res %>% filter(recovery == "oxic") %>% pull("aO"),
