...
plot_symmetry_measures <- function(res, 
  ...
  plot_df <- data.frame(aO = as.numeric(select(measures[species,], c(1, 2, 5, 6))),
                        state = as.numeric(select(measures[species,], c(3, 4, 7, 8))),
                        measure = as.character(colnames(measures)[c(1, 2, 5, 6)])) 
  ribbon_temp <- res %>% filter((aO >= median(measures$ox_TP)), (aO <= median(measures$anox_TP))) %>% select(c(species, recovery, aO))
  ribbon_df <- data.frame(oxic = rev(ribbon_temp %>% filter(recovery == "oxic") %>% pull(species)), 
