...
#' @autoglobal
#'
#' @importFrom dplyr filter mutate row_number case_when starts_with
#' @importFrom ggplot2 ggplot aes facet_wrap xlab ylab geom_line scale_colour_manual guides guide_legend scale_x_continuous sec_axis ylim
...
plot_temporal_ss <- function(temporal_results) {
  ...
  if ("aS" %in% colnames(temporal_results)) {
    if (length(unique(temporal_results$aS)) > 1) {
      ...
      t_func <- function(a) {
        approx(
          x = unique(temporal_results$aO),
          y = unique(temporal_results$aS),
          xout = a,
          method = "linear", rule = 1
        )$y # [1:length(temporal_results$aO)] # slice a random 0 element
      }

      p_substrates <- p_substrates + scale_x_continuous(
        expand = c(0, 0),
        sec.axis = sec_axis(
          trans = . ~ t_func(.),
      ...
      p_organisms <- p_organisms + scale_x_continuous(
        expand = c(0, 0),
        sec.axis = sec_axis(
          trans = . ~ t_func(.),
          name = expression(Log[10](Sulfide ~ diffusivity))
